function installPiecewisePolynomialToolbox(replaceExisting)
% INSTALLPIECEWISEPOLYNOMIALTOOLBOX installs Piecewise Polynomial Toolbox for MATLAB.
%   INSTALLPIECEWISEPOLYNOMIALTOOLBOX installs Piecewise Polynomial Toolbox into the following 
%   locations:
%                                Source: Destination
%   PiecewisePolynomialToolboxFunctions: matlabroot\toolbox\piecewisepolynomial
%
%   INSTALLPIECEWISEPOLYNOMIALTOOLBOX(true) installs Piecewise Polynomial Toolbox regardless of
%   whether a copy of the Piecewise Polynomial toolbox exists in the MATLAB root.
%
%   INSTALLPIECEWISEPOLYNOMIALTOOLBOX(false) installs Piecewise Polynomial Toolbox only if no copy 
%   of the Piecewise Polynomial toolbox exists in the MATLAB root.
%
...
if isToolbox == 7
    ...
    switch choice
        case 'Yes'
            ...
            if isRemoved
                fprintf('Previous version of Piecewise Polynomial Toolbox removed successfully.\n');
            else
                fprintf('Failed to remove old Piecewise Polynomial Toolbox folder:\n\t"%s"\n',toolboxRoot);
                fprintf(adminSolution);
                error(msgID,msg);
            end
        case 'No'
            fprintf('Piecewise Polynomial Toolbox currently exists, installation cancelled.\n');
            return
        case 'Cancel'
...
if isDir
    fprintf('Piecewise Polynomial toolbox folder created successfully:\n\t"%s"\n',toolboxRoot);
else
...
if ~isdir(toolboxContent)
    error(sprintf(...
        ['Change your working directory to the location of "installPiecewisePolynomialToolbox.m".\n',...
         '\n',...
         'If this problem persists:\n',...
         '\t(1) Unzip your original download of "PiecewisePolynomialToolbox" into a new directory\n',...
         '\t(2) Open a new instance of MATLAB "as administrator"\n',...
         '\t\t(a) Locate MATLAB shortcut\n',...
         '\t\t(b) Right click\n',...
         '\t\t(c) Select "Run as administrator"\n',...
         '\t(3) Change your "working directory" to the location of "installPiecewisePolynomialToolbox.m"\n',...
         '\t(4) Enter "installPiecewisePolynomialToolbox" (without quotes) into the command window\n',...
         '\t(5) Press Enter.']));
end
files = dir(toolboxContent);
wb = waitbar(0,'Copying Piecewise Polynomial Toolbox toolbox contents...');
n = numel(files);
fprintf('Copying Piecewise Polynomial Toolbox contents:\n');
for i = 1:n
